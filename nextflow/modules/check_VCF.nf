...
process checkVCF {
  ...
  */
  ...
  input:
  tuple val(id), path(vcf), path(vcf_index), path(vep_config)
  
  output:
  tuple val(id), path("*.gz", includeInputs: true), path ("*.gz.{tbi,csi}", includeInputs: true), path(vep_config)

  afterScript "rm -f *.vcf *.vcf.tbi *.vcf.csi"
