...
run_region() {
  ...
  # --------------------------------------------------------------------------
  # Compute bounding interval from first and last region
  # Format assumed: chr:start-end
  # --------------------------------------------------------------------------
  local first="${REG_ARR[0]}"
  local last="${REG_ARR[-1]}"
  ...
  # REG_ARR is already defined as array of chr:start-end entries
  ...
  for region in "${REG_ARR[@]}"; do
      ...
      if [[ -z "${SEEN[$chr]+x}" ]]; then
          # First time we see this chromosome
          MINPOS[$chr]=$start
          MAXPOS[$chr]=$end
          SEEN[$chr]=1
      else
          # Update bounding interval
          (( start < MINPOS[$chr] )) && MINPOS[$chr]=$start
          (( end   > MAXPOS[$chr] )) && MAXPOS[$chr]=$end
      fi
  done
  
  # Build BIG_REGION (one or many chr:start-end intervals)
  BIG_REGION=""
  for chr in "${!SEEN[@]}"; do
      interval="${chr}:${MINPOS[$chr]}-${MAXPOS[$chr]}"
      BIG_REGION="${BIG_REGION:+$BIG_REGION }$interval"
  done

  
  echo "Creating bamfiles for $BIG_REGION"
  local MERGED_BAMS=()

  # If HEADER==1, grab the header once from the first region
  if [[ "$HEADER" -eq 1 ]]; then
     for cram in "${CRAMS[@]}"; do
       local outbam="${WORKDIR}/$(basename "${cram%.*}")_${safe}.merged.bam"

       samtools view \
         --reference "$REFERENCE_FASTA" \
         -b -o "$outbam" \
		 --write-index \
         "$cram" chr1:10000-10002
      ...
      $MINPILEUP_ARGS \
      -r "$key" \
      "${MERGED_BAMS[@]}" | grep '^#' > "$HEADER_VCF"

    # Cleanup merged BAMs after header extraction
    for b in "${MERGED_BAMS[@]}"; do
      rm -f "$b" "${b}.csi"
    done
  ...
  : > "$group_vcf"
	  ...
	  for region in "${REG_ARR[@]}"; do
	    # run minipileup using the prebuilt BAMs
	    minipileup -f "$REFERENCE_FASTA" \
	      $MINPILEUP_ARGS \
	      -r "$region" \
	      "${MERGED_BAMS[@]}" | grep -v '^#' >> "$group_vcf" || true
	  done
	
	  # Final cleanup after all regions processed
	  for b in "${MERGED_BAMS[@]}"; do
	    rm -f "$b" "${b}.csi"
	  done
