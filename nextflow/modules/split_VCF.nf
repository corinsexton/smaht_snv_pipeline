...
process splitVCF {
  ...
  */
  ...
  input:
  tuple val(id), path(vcf), path(vcf_index), path(split_file), path(vep_config)

  output:
  tuple val(id), val("${vcf}"), path("${prefix}*.vcf.gz"), path("${prefix}*.vcf.gz.{tbi,csi}"), path(vep_config)
